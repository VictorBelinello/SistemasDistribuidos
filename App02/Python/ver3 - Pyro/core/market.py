...
import stock_client as StockClient
...
class StockMarket(object):
  ...
  # Atributos com _ não são expostos pelo Pyro
  _must_shutdown = False
  
  def __init__(self, name, companies):
    self._name = name
    print(f"Inicializando StockMarket {self._name}")
    ...
    threading.Thread(target=self.updateCompaniesQuotes, daemon=True).start()

    threading.Thread(target=self.updateTransactionsTimers, daemon=True).start()
  ...
  def initClient(self, client):
    """Inicializa as variáveis relacionadas a um novo cliente"""
    print("Inicializando novo cliente")
    try:
      self.clients_quotes[client.name] = []
      self.clients_subscriptions[client.name] = []
      self.clients[client.name] = client
      print(f"Cliente {client.name} inicializado")
    except Exception as e:
      raise ValueError(f"Problema de inicialização do cliente: {e}")

    
  
  def shutdown(self):
    """Chamado (pelo cliente) para sinalizar desligamento do servidor."""
    self._must_shutdown = True

  ###############################################################
  def getCompanyQuotesFor(self, name):
    print(f"{name} pedindo lista cotações")
    return self.clients_quotes[name]

  def addCompanyQuoteFor(self, name, company):
    print(f"{name} pedindo para adicionar {company}")
    quotes = self.clients_quotes[name]
    quotes.append( company )

  def removeCompanyQuoteFor(self, name, company):
    print(f"{name} pedindo para remover {company}")
    quotes = self.clients_quotes[name]
    for quote in quotes:
      print(quote)
    try:
  ...
  def addStockTransactionFor(self, client_name, stock_transaction):
    ...
    transaction_type = "buying" if stock_type == StockCompany.BUY_STOCK_TYPE else "selling"
  ...
  def updateCompaniesQuotes(self):
    ...
    while True:
      for company in self.companies.values():
        company['stock_value'] = StockCompany.getRandomStockValue()
