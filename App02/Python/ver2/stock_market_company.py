...
class Company(object):
    ...
    def __init__(self, name="Default"):
        ...
        self._stock_value = round(random.uniform(
            Company.MIN_STOCK_VALUE, Company.MAX_STOCK_VALUE), 2)
        ...
        if not Company._update_thread_running:
            Company._update_thread = Thread(
                target=Company.updateQuotes, daemon=True).start()
    ...
    def updateQuotes(cls):
        ...
        while True:
            for instance in cls._instances:
                instance._stock_value = round(random.uniform(
                    cls.MIN_STOCK_VALUE, cls.MAX_STOCK_VALUE), 2)
